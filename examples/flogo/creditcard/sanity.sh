...
response1=$(curl -X PUT http://localhost:7777/cibildata -H 'Content-Type: application/json' -d '{"id":"4","name":"Sam4","creditScore":"850"}' --write-out '%{http_code}' --silent --output /dev/null)
kill -9 $pId
...
response=$(curl -X PUT http://localhost:7777/applicant -H 'Content-Type: application/json' -d '{"name":"Sam4","age":"26","salary":"50100","address":"SFO","id":"4"}' --write-out '%{http_code}' --silent --output /dev/null)
response1=$(curl -X PUT http://localhost:7777/cibildata -H 'Content-Type: application/json' -d '{"id":"4","name":"Sam4","creditScore":"650"}' --write-out '%{http_code}' --silent --output /dev/null)

kill -9 $pId
if [ $response -eq 200  ] && [ $response1 -eq 200  ] && [[ "echo $(cat /tmp/testcase2.log)" =~ "lower cibil score" ]]
    then 
        echo "PASS"
    else
        echo "FAIL"
