...
func example(redis bool) error {
	...
	if events["checkSameEnvName"] != 3 {
		return fmt.Errorf("checkSameEnvName should have been called thrice")
	}
	if events["checkSameEnvNameAction"] != 1 {
		return fmt.Errorf("checkSameEnvNameAction should have been called once")
	}
...
func checkSameEnvName(ruleName string, condName string, tuples map[model.TupleType]model.Tuple, ctx model.RuleContext) bool {
	...
	name1 := os.Getenv("name")
	events := ctx.(map[string]int)
	count := events["checkSameEnvName"]
	events["checkSameEnvName"] = count + 1
	return name == name1
}
