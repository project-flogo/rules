...
func main() {
	...
	fmt.Printf("Rule added: [%s]\n", rule2.GetName())
	rule2.AddCondition("c1", []model.StreamSource{"n1"}, checkForTom) // check for name "Bob" in n1
	//in effect, fire the rule when name field in both tuples in "Bob"
	rule2.SetActionFn(checkForTomAction)
	rule2.SetPriority(100)

	//Create Rule, define conditiond and set action callback
	rule3 := ruleapi.NewRule("2* name == Tom *")
	fmt.Printf("Rule added: [%s]\n", rule3.GetName())
	rule3.AddCondition("c1", []model.StreamSource{"n1"}, checkForTom) // check for name "Bob" in n1
	//in effect, fire the rule when name field in both tuples in "Bob"
	rule3.SetActionFn(checkForTomAction2)
...
func checkForTom(ruleName string, condName string, tuples map[model.StreamSource]model.StreamTuple) bool {
	//This conditions filters on name="Bob"
	streamTuple := tuples["n1"]
	if streamTuple == nil {
		fmt.Println("Should not get a nil tuple in FilterCondition! This is an error")
