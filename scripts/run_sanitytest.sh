...
function execute_testcase()
{
    ...
    do
        ...
        if [[ $TESTCASE == *"PASS"* ]];  then
            ...
            sed -i "s/<\/tr> <\/table>/<tr><td>$RECIPE<\/td><td>${TESTCASE_LIST[i]}<\/td><td  class="success">PASS<\/td><\/tr><\/tr> <\/table>/g" $GOPATH/$FILENAME
        else
            echo "$RECIPE":"Failed"
            FAIL_COUNT=$((FAIL_COUNT+1))
            sed -i "s/<\/tr> <\/table>/<tr><td>$RECIPE<\/td><td>${TESTCASE_LIST[i]}<\/td><td  class="error">FAIL<\/td><\/tr><\/tr> <\/table>/g" $GOPATH/$FILENAME
        fi
    done
}
...
sed -i s/"passed <\/td> <td>"/"passed <\/td> <td>$PASS_COUNT"/g $GOPATH/$FILENAME
sed -i s/"failed <\/td> <td>"/"failed <\/td> <td>$FAIL_COUNT"/g $GOPATH/$FILENAME
sed -i s/"cases<\/td><td>"/"cases<\/td><td>$((PASS_COUNT+FAIL_COUNT))"/g $GOPATH/$FILENAME